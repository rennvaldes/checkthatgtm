...
export default function ArticlePageClient({
  ...
  const articleData = React.useMemo(
    () => (rawData ? getCardFromStrapiRawData(rawData.data) : {}),
    [rawData]
  );
  const hasKeepReading =
    Array.isArray((articleData as any).related_articles) &&
    (articleData as any).related_articles.length > 0;
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`w-full max-w-[1350px] mt-16 lg:mt-24 ${
          hasKeepReading ? "mb-4 lg:mb-12" : "lg:-mb-24"
        }`}
      >
        <CTABanner
          logoSrc={GrowthXLogo}

...
export function NavigationBar() {
  ...
  return (
    <>
      <nav 
        className={cx(
          "w-full h-fit pt-16 md:h-16 md:pt-0 sticky top-0 z-50",
      ...
      >
        <Grid className="h-full items-center">
          ...
          <div className="hidden md:flex col-span-4 md:col-start-3 items-center gap-2 lg:gap-3 h-full">
            <Link
              href="/about"
              className={cx(
      ...
      <div
        aria-hidden={!isOpen}
        className="bg-background aria-hidden: group fixed left-0 top-0 z-[100] flex h-screen w-screen flex-col overflow-hidden transition-all duration-500 aria-hidden:translate-x-[100%] aria-hidden:opacity-50"
      >
        ...
        <div className="flex flex-1 flex-col justify-between items-center px-[20px] pb-[40px] pt-[40px] transition-transform duration-1000 group-aria-hidden:translate-x-[15%]">
          <ul className="flex flex-col items-center gap-[24px]">
            <li onClick={() => setIsOpen(false)}>
              <Link
                ...
                className={cx(
                  ...
                  pathname === "/about" ? "text-foreground/40" : "text-foreground/40 hover:text-foreground"
                )}
              >
                Company
              </Link>
            ...
            <li onClick={() => setIsOpen(false)}>
              <Link
                ...
                className={cx(
                  "text-[28px] leading-[31px] transition-colors",
                  pathname === "/careers" ? "text-foreground/40" : "text-foreground/40 hover:text-foreground"
                )}
              >
                Careers
              </Link>
          ...
          <Link
            href='/book-demo'
            onClick={() => {
              trackDemoBookingClick('navbar', 'mobile');
              setIsOpen(false);
            }}
            className="w-full inline-flex h-9 items-center justify-center gap-2 rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground transition-opacity hover:opacity-80"
          >
            Book a demo
